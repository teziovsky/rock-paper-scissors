<script lang="ts">
	...
	const previouslyFocused = (typeof document !== 'undefined' && document.activeElement) as
		| HTMLElement
		| false
		| null;
...
{#if showRules}
	<div
		...
		class="fixed flex items-center justify-center inset-0 md:right-auto md:bottom-auto flex-col gap-10 z-50 md:top-[45%] bg-white p-8 md:rounded-xl md:-translate-y-1/2 md:left-1/2 md:-translate-x-1/2"
		role="dialog"
		aria-modal="true"
		bind:this={modal}
	>
		<div class="flex items-center justify-between w-full">
			<h2
				class="absolute md:top-[unset] md:left-[unset] md:translate-x-[unset] md:translate-y-[unset] text-4xl text-center uppercase -translate-x-1/2 -translate-y-1/2 top-1/4 left-1/2 md:static md:text-left text-dark"
			>
				rules
			</h2>
			<button
				class="absolute md:static md:left-[unset] md:translate-x-0 md:bottom-[unset] p-2 transition-colors -translate-x-1/2 rounded-lg text-grey enabled:hover:text-dark enabled:focus-within:text-dark enabled:focus-within:outline-dashed enabled:focus-within:outline-dark enabled:focus-within:outline-offset-4 enabled:focus-within:outline-[3px] bottom-16 left-1/2"
				on:click={close}
			>
